...
class Product(models.Model):
    ...
    def save(self, *args, **kwargs):
        if not self.slug:
            base_slug = slugify(f"{self.brand.name}-{self.name}-{self.model_number}") or "product"
            slug = base_slug
            counter = 2
            # Guarantee uniqueness even when two different names collapse to the
            # same slug (e.g. "Pro+" and "Pro" both lose their "+" in slugify).
